...
过拟合问题往往是由于<u>训练数据少，噪声大以及模型能力强</u>等原因造成的。为了解决过拟合问题，一般采用限制模型能力的方法——在经验风险最小化的基础上再引入参数的**正则化(regularization)**，使经验风险不要过度地最小化。这种准则就是**结构风险最小化(structure risk minimization, SRM)**准则。
$$
...
为了充分利用凸优化中一些高效、成熟的优化算法，比如梯度下降法、共轭梯度、拟牛顿法等，很多机器学习方法都倾向于选择合适的模型和损失函数，以<u>构造一个凸函数作为优化目标</u>。但也有很多模型(比如神经网络)的优化目标是非凸的，只能退而求其次找到局部最优解。
...
**特征选择(feature selection)**是选取原始特征集合的一个有效子集，使得基于这个特征子集训练出来的模型准确率最高。简单地说，特征选择就是<u>保留有用特征，移除冗余或无关的特征</u>。
...
$$
从上面的公式可以看出，<u>模型越复杂，即假设空间$$\mathcal{F}$$越大，那么模型的泛化能力越差</u>；<u>要达到相同的泛化能力，越复杂的模型需要的样本数量越多</u>。<u>为了提高模型的泛化能力，通常需要正则化来限制模型复杂度</u>。
